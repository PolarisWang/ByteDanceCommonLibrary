...
namespace ByteDance.Foundation
{
    public abstract class MonoSingletonExt<T> : MonoBehaviour where T : MonoSingletonExt<T>
    {
        ...
        public static T instance
        {
            get
            {
                ...
                if (m_Instance == null)
                {
                    ...
                    if (m_Instance == null)
                    {
                        MyLogger.Log.LogWarning(string.Format(STR_NoInstanceOfATemporaryOneIsCreated.F(typeof(T).ToString())));
                        m_Instance = new GameObject(STR_TempInstanceOf + typeof(T).ToString(), typeof(T))
                            .GetComponent<T>();

                        // Problem during the creation, this should not happen
                        if (m_Instance == null)
                        {
                            MyLogger.Log.LogError(string.Format(STR_ProblemDuringTheCreationOf.F(typeof(T).ToString())));
                        }
                    }
        ...
        private void Awake()
        {
            ...
            // TESTING
            MyLogger.Log.LogInfo(string.Format("Awaking: {0}".F(this.name)));
        }
        ...
        {
            IsShuttingDown = true;
            MyLogger.Log.LogInfo(string.Format("Quitting: {0}".F( this.name)));
